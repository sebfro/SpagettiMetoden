...
!    real :: deltaX, deltaY
    ...
    do n = 1, nFish
      if (life(n) == 1) then  ! Only consider active fish
       ...
       ! Depth check
       depthOK = .true.
       if ((tag_depth(tstep) > 1.2*H + 50.0) .and. (tag_depth(tstep) < 5000)) then
         depthOK = .false.
       end if
       ! Check land
       if (H < 0) then
       ...
       tempOK = .True.
       !if ((tag_temp(tstep) < 500.0) .and. (abs(tag_temp(tstep) - sst) > 3.0)) then
       !   tempOK = .False.
       !end if
       ...
       else ! New position does not work
