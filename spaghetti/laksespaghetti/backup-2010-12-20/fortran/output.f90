...
!use topography
use time_module, only: datestr
use setup, only: forward_outfile_name, backward_outfile_name
use setup, only: start_time, simdays
...
subroutine init_FW_output
  ...
  open(51, file='terminateFW.dat', form='formatted', action='write')
end subroutine init_FW_output

subroutine init_BW_output
  call init_output(backward_outfile_name, BWid)
  open(52, file='terminateBW.dat', form='formatted', action='write')
...
! Global attributes 
  ...
  if (fname == forward_outfile_name) then
    status = nf90_put_att(ncid, NF90_GLOBAL, 'type',        & 
             'Spaghetti forward  output file')
  else
    status = nf90_put_att(ncid, NF90_GLOBAL, 'type',        & 
             'Spaghetti backward output file')
...
!
  subroutine save_output(ncid, daynr)
    ...
    if (ncid == BWid) then
       daynr2 = simdays - daynr   ! Counting down
    else
       daynr2 = daynr
    end if

    status = nf90_put_var(ncid, time_id, daynr2, start = (/ daynr+1 /))
    call check_err(status)
